  ...
  for group in ['dev','eval']:
    ...
    for tc in models_ids:
      ...
      for split_id in range(0,N_MAX_SPLITS):
        ...
        split_path = os.path.join(config.zt_norm_B_dir, group, str(tc) + "_" + str(split_id).zfill(4) + ".hdf5")
        if split_id == 0 and not os.path.exists(split_path):
          ...
          break
        B_tc = bob.io.load(split_path)
        if not started:
          B = B_tc
          started = True
        else:
          B = np.concatenate((B, B_tc), 1)
      # Saves to files
      bob.io.save(B, os.path.join(config.zt_norm_B_dir, group, str(tc) + ".hdf5"))
