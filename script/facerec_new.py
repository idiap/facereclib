...
def default_tool_chain(args):
  ...
  pp = imp.load_source('pp', args.preprocessing)
  ...
  tool_chain.preprocess_images(pp, args.force)
...
def main():
  ...
  parser.add_argument('-p', '--preprecessing-config', metavar = 'FILE', type = str, dest='preprocessing', 
                      help = 'Configuration script for preprocessing the images')
  
  parser.add_argument('--trained-model-file', type = str, metavar = 'FILE', default = 'Extractor.hdf5',
  ...
  parser.add_argument('-n', '--preprocessing', type = int, nargs = 2, metavar = 'POS', 
  ...
  parser.add_argument('-f', '--feature-extraction', type = int, nargs = 2, metavar = 'POS', 
                      help = 'Perform feature extraction for the given range of preprocessed images')
  
  parser.add_argument('-m', '--model-generation', type = int, nargs = 2, metavar = 'POS', 
                      help = 'Generate the given range of models from the features')
  
  parser.add_argument('-s', '--compute-scores', type = int, nargs = 2, metavar = 'POS', 
                      help = 'Compute scores for the given range of models')
  
  parser.add_argument('-S', '--score-type', type = str, choices=['A', 'B', 'C', 'D', 'Z'],  metavar = 'SCORE', 
