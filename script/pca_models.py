...
def main():
  ...
  for model_id in models_ids:
    ...
    if os.path.exists(model_path):
      ...
      enrol_files = db.files(directory=config.featuresProjected_dir, extension=config.featuresProjected_ext, 
                             groups=args.group, protocol=config.protocol, model_ids=(model_id,), purposes='enrol')
      pca.pca_enrol_model(enrol_files, model_path)
