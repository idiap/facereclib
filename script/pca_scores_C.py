...
def main():
  ...
  probe_files = db.objects(directory=config.featuresProjected_dir, extension=config.featuresProjected_ext, groups=(args.group,), protocol=config.protocol, purposes="probe")
  ...
  for tmodel_id in tmodels_ids:
    pca.pca_ztnorm_C(tmodel_id, config.tnorm_models_dir, probe_files, db, 
