...
def main():
  ...
  probe_files = db.files(directory=config.gmmstats_dir, extension=config.gmmstats_ext, groups=(args.group,), protocol=config.protocol, purposes="probe")
  ...
  for tmodel_id in tmodels_ids:
    jfa.jfa_ztnorm_C(tmodel_id, config.tnorm_models_dir, probe_files, config.jfabase_filename, config.ubm_filename, db, 
