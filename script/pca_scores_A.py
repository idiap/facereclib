...
def getModel(model_id, models_dir):
  model_path = os.path.join(models_dir, str(model_id) + ".hdf5")
  if not os.path.exists(model_path):
    raise RuntimeError, "Could not find model %d" %(model_path)
  return bob.io.load(model_path)
...
def main():
  ...
  for model_id in models_ids:
    ...
    probe_files = db.objects(directory=config.featuresProjected_dir, extension=config.featuresProjected_ext, groups=(args.group,), protocol=config.protocol, purposes="probe", model_ids=(model_id,))
    ...
    import pca
    pca.pca_scores_A((model_id,), config.models_dir, probe_files, db, 
