...
def main():
  ...
  if args.nogrid: subprocess.call(cmd_pca)
  else:
  ...
  n_array_jobs = config.TOTAL_ARRAY_JOBS
  cmd_pca_project = [ 
              'pca_project.py', 
              '--config-file=%s' % args.config_file, 
              '--grid'
            ]
  if args.nogrid: subprocess.call(cmd_pca_project)
  else:
    job_pca_project = submit(jm, cmd_pca_project, dependencies=[job_pca.id()], array=(1,n_array_jobs,1))
  ...
  for group in ['dev','eval']:
    ...
    if args.nogrid: subprocess.call(cmd_scores_A)
      ...
      job_scores_int = submit(jm, cmd_models, dependencies=deps, array=(1,n_array_jobs,1))
  ...
  if config.zt_norm:
    for group in ['dev','eval']:
      ...
      if args.nogrid: subprocess.call(cmd_scores_B)
        ...
        job_scores_int = submit(jm, cmd_scores_B, dependencies=deps, array=(1,n_array_jobs,1), queue='q1d', mem='3G', hostname='!beaufix&!cicatrix')
  ...
  if config.zt_norm:
    for group in ['dev','eval']:
      ...
      if args.nogrid: subprocess.call(cmd_scores_C)
        ...
        job_scores_int = submit(jm, cmd_scores_C, dependencies=deps, array=(1,n_array_jobs,1), queue='q1d', mem='3G', hostname='!beaufix&!cicatrix')
  ...
  if config.zt_norm:
    for group in ['dev','eval']:
      ...
      if args.nogrid: subprocess.call(cmd_scores_D)
        ...
        job_scores_int = submit(jm, cmd_scores_D, dependencies=deps, array=(1,n_array_jobs,1), queue='q1d', mem='3G', hostname='!beaufix&!cicatrix')
