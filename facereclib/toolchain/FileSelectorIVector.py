...
class FileSelectorIVector:
  ...
  def __init__(
        ...
        projected_directory,
        enroller_file,
        projected_ivec_directory,
        whitening_projector_file,
        whitening_projected_directory,
        model_directories,
        score_directories,
    ...
    self.projected_ivec_directory = projected_ivec_directory
    self.whitening_projector_file = whitening_projector_file
    self.whitening_projected_directory = whitening_projected_directory
  ...
  def get_paths(self, files, directory_type = None, directory = None, extension = None):
    ...
    elif directory_type == 'projected_ivec':
      directory = self.projected_ivec_directory
    elif directory_type == 'whitening_projected':
      directory = self.whitening_projected_directory
  ...
  def projected_ivec_list(self):
    """Returns the list of projected feature files."""
    return self.get_paths(self.m_database.all_files(), "projected_ivec")

  def whitening_projected_list(self):
    """Returns the list of projected feature files."""
    return self.get_paths(self.m_database.all_files(), "whitening_projected")
  ### Training lists
  ...
  ### Training lists
  def training_whitening_list(self):
    """Returns the list of ivectors that should be used for projector training."""
    files = self.m_database.training_files('train_projector', False)
    print files[0]

    return self.get_paths(files, 'projected_ivec')
