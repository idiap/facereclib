...
class FileSelectorGBU:
  ...
  def original_image_list(self, set):
    ...
    return self.m_db.files(directory=self.m_config.image_directory, extension=self.m_config.image_extension, protocol=self.m_config.protocol, **opts)
  ...
  def training_feature_list(self, dir_type):
    ...
    # query the database
    return self.m_db.files(directory=cur_dir, extension=self.m_config.default_extension, protocol=self.m_config.protocol, **self.__options__('training', 'extractor_training_options'))
  ...
  def training_feature_list_by_clients(self, dir_type):
    ...
    if dir_type == 'preprocessed':
      cur_dir = self.m_config.preprocessed_dir
      cur_world_options = self.__options__('training', 'extractor_training_options')
    elif dir_type == 'features':
      cur_dir = self.m_config.features_dir
