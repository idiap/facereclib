...
class FileSelectorZT:
  ...
  def original_image_list(self):
    ...
    return self.m_db.files(directory=self.m_config.image_directory, extension=self.m_config.image_extension, protocol=self.m_config.protocol, **self.__options__('all_files_options'))

  def annotation_list(self):
    """Returns the list of annotation files, if any (else None)"""
    if not hasattr(self.m_config, 'annotation_directory') or self.m_config.annotation_directory == None:
      return None

    return self.m_db.files(directory=self.m_config.annotation_directory, extension=self.m_config.annotation_extension, protocol=self.m_config.protocol, **self.__options__('all_files_options'))
  ...
  def training_image_list(self):
    ...
    return self.m_db.files(directory=self.m_config.preprocessed_dir, extension=self.m_config.default_extension, protocol=self.m_config.protocol, groups='world', **self.__options__('extractor_training_options'))

  def training_feature_list(self):
    """Returns the list of features that should be used for projector training"""
    return self.m_db.files(directory=self.m_config.features_dir, extension=self.m_config.default_extension, protocol=self.m_config.protocol, groups='world', **self.__options__('projector_training_options'))
