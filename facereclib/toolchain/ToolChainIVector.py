...
class ToolChainIVector:
  ...
  def train_whitening(self, tool, extractor, force=False):
    """Train the feature projector with the extracted features of the world group."""
    if tool.requires_projector_training:
      ...
      if self.__check_file__(projector_file, force, 1000):
        ...
        # perform training
        tool.train_whitening(train_files, str(projector_file))


  def enroll_models(self, tool, compute_zt_norm, indices = None, groups = ['dev', 'eval'], types = ['N','T'], force=False):
      ...
      for group in groups:
        ...
        for model_id in model_ids:
          ...
          if not self.__check_file__(model_file, force):
            enroll_files = self.m_file_selector.enroll_files(model_id, group, 'whitening_projected')

            # load all files into memory
            enroll_features = [bob.io.load(str(enroll_file)) for enroll_file in enroll_files]
    ...
    for model_id in model_ids:
      ...
      if self.__check_file__(score_file, force):
        ...
        if preload_probes:
          ...
          current_probe_files = self.m_file_selector.get_paths(current_probe_objects, 'whitening_projected')
