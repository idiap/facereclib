...
class FileSelector:
  ...
  ### Training lists
  def training_list(self, directory_type, step, sort_by_client = False):
  ...
  def t_enroll_files(self, model_id, group, directory_type):
    """Returns the list of T-norm model features used for enrollment of the given model_id from the given group"""
  ...
  def c_file(self, model_id, group):
    ...
    return os.path.join(c_dir, "TM" + str(model_id) + self.default_extension)
  ...
  def d_file(self, model_id, group):
    ...
    return os.path.join(d_dir, str(model_id) + self.default_extension)
