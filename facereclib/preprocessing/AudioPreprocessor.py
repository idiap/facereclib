...
class AudioPreprocessor:
  """Do Audio preprocessing like using external VAD segmentation"""
  def __init__(self, 
      win_length_ms = 20,
      win_shift_ms = 10,
      n_filters = 24,
  ...
  def read_original_image(self, filename):
  ...
  def __call__(self, rate_wavsample, annotations = None):
    ...
    return rate_wavsample


  def save_image(self, wav_sample, wav_file):
    hdf5file = bob.io.HDF5File(wav_file, "w")
    rate = wav_sample[0]
    signal = wav_sample[1]
    hdf5file.cd('/')
    hdf5file.set('rate', rate)
    hdf5file.set('data', signal)
    

  def read_image(self, wav_name):
    hdf5file = bob.io.HDF5File(wav_name)
    hdf5file.cd('/')
    rate = hdf5file.read('rate')
    data = hdf5file.read('data')
    return [rate, data] 
